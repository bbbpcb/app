...
class Task_wenti_mdl extends  CI_Model
{



    const TABLE ='dw_task_wenti';  
    ...
    public function getList($config=array())
    {
        ...
        $list = array();
        $list = $this->db->select('wenti.*,m.realname,m.headerurl')
                        ->from(self::TABLE.' as wenti')
                        ->join(self::TABLE_MEMBER.' as m','m.id=wenti.mid','left')
                        ->get()->result_array();
