...
class Question_reply_mdl extends  CI_Model
{

    const TABLE = 'dw_question_reply';
    const TABLE_MEMBER = 'dw_expert';
   // const TABLE_DEPARTMENT ='dw_department';  
    ...
    public function getList($config = array())
    {
        ...
        $list = array();
        $list = $this->db->select('qr.*,ex.username,ex.realname,ex.headerurl')
                ->from(self::TABLE.' as qr')
                ->join(self::TABLE_MEMBER.' as ex','ex.id=qr.exid','left')
                ->get()
                ->result_array();
