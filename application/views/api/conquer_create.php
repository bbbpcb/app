...
<section id="main-info">
  <header class="aui-bar aui-bar-nav aui-bar-warning" id="aui-header"> <a class="aui-pull-left aui-btn <?php if(!$_GET['cid']){?>abs<?php }?>" href="javascript:;window.history.go(-1)"> <span class="aui-iconfont aui-icon-left"></span> </a>
    <?=$_GET['cid']?'修改':'创建'?>
    <?php if($_GET['cid']){?>
    <a class="aui-pull-right aui-btn " href="javascript:;" onClick="delproject(<?=$_GET['cid']?>)">删除</a>
    <?php } ?>
  </header>
    ...
    <div class="page-main">
      <div class="from-main">
        ...
        <div class="list"> <span class="name">复盘描述</span>
          <input type="text" class="inp" value="<?=$proinfo['content']?>" name="intro" id="content" placeholder="请输入项目描述" />
        </div>
        <div class="list"> <span class="name">添加配图</span>
          <div class="pic-box">
            ...
            <div id="tupian1" style="position:absolute">
            <?php if($_GET['cid']){?>
             <img src="<?=base_url()?>/uploads/conquer/<?=$proinfo['icon']?>" style="    float: right;
    width: 50%;" /><input value='<?=$proinfo['icon']?>' type='hidden' id='icon' />
             <?php } ?>
        ...
        <div class="list"> <span class="name">最优秀回答获得分数</span>
          <input type="text" class="inp" value="<?=$proinfo['total']?>" name="total" id="total" placeholder="请输入分数" />
        </div>
      </div>
    </div>
...
var toast = new auiToast({})
 ...
 $("#basics").click(function(){
    ...
    <?php if($_GET['cid']){?>
	 var url = "<?=base_url();?>index.php?d=api&c=conquer&m=conquer_update&cid=<?=$_GET['cid']?>";
	<?php }else{ ?>
 ...
 function delproject(id){
  ...
  var url = "<?=base_url();?>index.php?d=api&c=conquer&m=delcon";
  var data = {'cid':id};
   $.post(url, data,
    function(res) {
        var res = jQuery.parseJSON(res);
