	...
	function judgeSelect() {
		...
		if (!result) {
			...
			if(confirm("提示：确认要删除选定的记录吗？")){
				$.ajax({
					type : "POST",
					url : "<?=base_url()?>index.php?d=home&c=review_question&m=delall",
					data : {ids:ids},
					cache:false,
					success : function(data){
						alert("删除成功");
						location.href = "<?=base_url()?>index.php?d=home&c=review_question&page=<?=$pagesize ?>&modid=<?=$modid ?>";
					}
				});
			}
		}
  ...
  <div class="container-fluid">
    ...
    <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
      <ul class="nav navbar-nav">
        <a href="<?=base_url()?>index.php?d=home&c=review_question&m=add&modid=<?=$modid?>"><li class="active"><button type="button" class="btn btn-default navbar-btn">添加</button></li></a>
...
<table class="table table-bordered table-hover definewidth m10">
    ...
    <tr>
        ...
        <td valign="middle"><?=$v['revtitle']?></td>
        <td valign="middle"><?=$v['revintro']?></td>
        <td>
		<a href="<?=base_url()?>index.php?d=home&c=review_question&m=update&id=<?=$v['id']?>">编辑</a> | 
		<a href="<?=base_url()?>index.php?d=home&c=review_question&m=del&id=<?=$v['id']?>&modid=<?=$v['modid']?>">删除</a>
		</td>
    </tr>
