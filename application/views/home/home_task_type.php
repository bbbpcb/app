	...
	function judgeSelect() {
		...
		if (!result) {
			...
			if(confirm("提示：确认要删除选定的记录吗？")){
				$.ajax({
					type : "POST",
					url : "<?=base_url()?>index.php?d=home&c=Tasktype&m=delall",
					data : {ids:ids},
					cache:false,
					success : function(data){
						alert("删除成功");
						location.href = "<?=base_url()?>index.php?d=home&c=Tasktype&page=<?=$pagesize ?>";
					}
				});
			}
		}
  ...
  <div class="container-fluid">
    ...
    <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
      <ul class="nav navbar-nav">
        <a href="<?=base_url()?>index.php?d=home&c=tasktype&m=add"><li class="active"><button type="button" class="btn btn-default navbar-btn">添加</button></li></a>

      </ul>
       <form class="navbar-form navbar-left" role="search" method="post"  action="<?=base_url()?>index.php?d=home&c=tasktype&m=index">
...
<table class="table table-bordered table-hover definewidth m10">
    ...
    <tr>
        ...
        <td valign="middle"><?php echo isset($tasktypecount[$v['id']]) ? $tasktypecount[$v['id']] : 0 ;?></td>
        <td valign="middle"><input type="text" name="rank[]" value="<?=$v['rank']?>" /></td>
        <td>
       
		<a href="<?=base_url()?>index.php?d=home&c=tasktype&m=update&id=<?=$v['id']?>">编辑</a> | 
		<a href="<?=base_url()?>index.php?d=home&c=tasktype&m=del&id=<?=$v['id']?>">删除</a>
		</td>
    </tr>
