	...
	function judgeSelect() {
		...
		if (!result) {
			...
			if(confirm("提示：确认要删除选定的记录吗？")){
				$.ajax({
					type : "POST",
					url : "<?=base_url()?>index.php?d=home&c=Pics&m=delall",
					data : {ids:ids},
					cache:false,
					success : function(data){
						alert("删除成功");
						location.href = "<?=base_url()?>index.php?d=home&c=Pics&page=<?=$pagesize ?>";
					}
				});
			}
		}
...
<table class="table table-bordered table-hover definewidth m10">
    ...
    <tr>
        ...
        <td valign="middle"><img src="<?=base_url()?>uploads/pics/<?=$v['picname']?>" width="40px" height="40px"/></td>
