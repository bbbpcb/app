	...
	function judgeSelect() {
		...
		if (!result) {
			...
			if(confirm("提示：确认要删除选定的记录吗？")){
				$.ajax({
					type : "POST",
					url : "<?=base_url()?>index.php?d=home&c=conquer&m=delall",
					data : {ids:ids},
					cache:false,
  ...
  <div class="container-fluid">
    ...
    <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
      <ul class="nav navbar-nav">
        <a href="<?=base_url()?>index.php?d=home&c=conquer&m=add"><li class="active"><button type="button" class="btn btn-default navbar-btn">添加</button></li></a>

      </ul>
       <form class="navbar-form navbar-left" role="search" method="post"  action="<?=base_url()?>index.php?d=home&c=conquer&m=index">
...
<table class="table table-bordered table-hover definewidth m10">
    ...
    <tr>
        ...
        <td style="display:none"><?=$v['content']?></td>
		<td><?=$v['total']?></td>
        <td>
