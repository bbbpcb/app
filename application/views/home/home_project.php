	...
	function judgeSelect() {
		...
		if (!result) {
			...
			if(confirm("提示：确认要删除选定的记录吗？")){
				$.ajax({
					type : "POST",
					url : "<?=base_url()?>index.php?d=home&c=project&m=delall",
					data : {ids:ids},
					cache:false,
					success : function(data){
						alert("删除成功");
						location.href = "<?=base_url()?>index.php?d=home&c=project&typeid=<?=$typeid ?>&page=<?=$pagesize ?>";
					}
				});
			}
		}
  ...
  <div class="container-fluid">
    ...
    <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
      <ul class="nav navbar-nav">
        <a href="<?=base_url()?>index.php?d=home&c=project&m=add"><li class="active"><button type="button" class="btn btn-default navbar-btn">添加</button></li></a>
      </ul>
      <form class="navbar-form navbar-left" role="search" method="post"  action="<?=base_url()?>index.php?d=home&c=project&m=index">
...
<table class="table table-bordered table-hover definewidth m10">
    ...
    <tr>
		...
		<td><?=$v['header']?></td>
      	<td><?=$v['status']?></td>
        <td>
    <a href="<?=base_url()?>index.php?d=home&c=task&m=index&pid=<?=$v['id']?>">任务列表</a> |
	...
	?>
	<tr>
        <td colspan="9"><div class="btn_check pt10" style="float:left"><input  onClick="selectAll(this)"  id="che" name="che" type="checkbox">全选/取消&nbsp;&nbsp;<a href="javascript:;" onClick="return judgeSelect();" class="btn_del">删除所选</a></div><div class="btn_check pt10" style="float:right"><?=$page?></div></td>
    </tr>
</table>
