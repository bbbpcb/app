...
class jobs extends Admin_Controller
{
	...
	public function index()
	{
        ...
        $count = $this->jobs->get_count1($where);
		...
		$this->load->view('home/home_jobs',$data);
	}
	...
	public function add()
	{
		...
		if(!empty($_POST)){
			...
			if(!empty($type_name)){
				...
				$this->jobs->add($add);
				redirect('d=home&c=jobs');
			}
		}else{
			$this->load->view('home/home_jobs_add');
		}
	...
	public function update()
	{
		if(!empty($_POST)){
			...
			$config = array('id'=>$id);
			$info = $this->jobs->get_one_by_id($config);

			$data['info'] = $info;
			$this->load->view('home/home_jobs_update',$data);
