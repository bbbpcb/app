...
class Task_fansi_mdl extends  CI_Model
{



    const TABLE ='dw_task_fansi';  
    ...
    public function getList($config=array())
    {
        ...
        $list = array();
        $list = $this->db->select('fansi.*,m.realname')
                        ->from(self::TABLE.' as fansi')
                        ->join(self::TABLE_MEMBER.' as m','m.id=fansi.mid','left')
                        ->get()->result_array();
